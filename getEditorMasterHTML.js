...
function masterHTMLBrowserCode(
    ...
    files,doc,getEl,docEl,qrySel,qryAll,default_theme,getLinks,modeFromFilename,setupSplitter
) {
    ...
    function windowOpen (prefix,file,suffix) {
        return window.open(prefix + (file ? encodeURIPath(resolveFile(file)) : '')+(suffix?suffix:''),"_blank", "scrollbars=1,fullscreen=yes,status=no,toolbar=no,menubar=no,location=no");
    }
    ...
    function serveFile (file) {
        if (files[file].serving) {
          if (menu_ws && typeof menu_ws.send==='function') {
              menu_ws.send(JSON.stringify({file:file,serve:false}));
          }  
        } else {
          if (menu_ws && typeof menu_ws.send==='function') {
              menu_ws.send(JSON.stringify({file:file,serve:true}));
          }  
        }
    ...
    function dirHtml(f_list, ids, root, ind) {
      ...
      f_list.forEach(function(f) {
        ...
        if (typeof f === "string") {
          ...
          h += ind + '<label data-file="'+fn+'" for="'+i+'">'+f+'&nbsp;<span class="show_theme">'+mod+"/"+thm+'</span><i class="fa fa-times-circle-o close_btn"></i><i class="fa fa-skyatlas serve_btn"></i></label>\n';
        } else {
            
          var dir = ids[i] = root + f.dir;
          
          h += ind + '<input type="checkbox" id="' + i + '">\n';
          h += ind + '<label data-dir="'+dir+'" for="' + i + '">' + f.dir + '<i class="fa fa-skyatlas serve_btn"></i></label>\n';
    ...
    function loadfiles_setEvents(el, html) {
        ...
        el.querySelectorAll("label").forEach(function(lab){
            if (lab.dataset.file) {
                ...
                lab.querySelector("i.serve_btn").addEventListener("click",fileTreeServeClick);
                lab.querySelector("i.close_btn").addEventListener("click",fileTreeCloseClick);
    ...
    function fileTreeServeClick(e) {
        serveFile (getDatasetField("file",e.target));
    }
    
    function fileTreeCloseClick(e) {
        closeFile (getDatasetField("file",e.target));
    }
    ...
    menu_ws.onmessage = function (evt) {
       ...
       if (payload.file && (file=files[payload.file]) ) {
            ...
            if (payload.serve===true) {
                
                el = file_index_fn2el(payload.file);
                if (el) {
                    
                    file.serving=windowOpen (ace_single_serve_file_url(payload.file));
                    var int = setInterval(function(){
                        if (file.serving && file.serving.closed) {
                            delete file.serving;
                            clearInterval(int);
                            payload.serve=false;
                            menu_ws.send(JSON.stringify(payload));
                        }
                    },500);
                    el.labels[0].classList.add("file_serving")
                }
                return;
            }
            
            if (payload.serve===false) {
                ...
                if (el) {
                    ...
                    el.labels[0].classList.remove("file_serving")
                }
                return;
            }
            ...
            if (payload.errors ===0 && file.serving && !file.serving.closed) {
                file.serving.location.reload();
            }
            /*fields.forEach(
                    update.bind(
                        this,
 ...
 return function getEditorMasterHTML (files,title,theme,faExpress,append_html) {
        ...
        return function getEditLaunchHtml(req,res) {
            ...
            html.append({
                ...
                ace_single_serve_dir_url  : ace_single_serve_dir_url,
                ace_single_serve_file_url : ace_single_serve_file_url,
                getLinks                  : getLinks,
                files                     : filesNow,
            ...
            html.append(require("./setupSplitter.js"),"body");
